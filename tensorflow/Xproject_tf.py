...
class ImageQA(object):
    ...
    def __init__(self, config):
        ...
        self.batch_size =tf.get_variable("batch_size", config.batch_size)
        ...
        if layers is not None:
            ...
            self.init_state = my_rnn.zero_state(self.batch_size, tf.float32)
        else:
            self.my_rnn = my_rnn = rnn_
            #self.init_state = tf.zeros([self.batch_size, my_rnn.state_size])
    ...
    def build_model(self):
        ...
        x = tf.placeholder(tf.int32, [None, self.steps])
        x_mask = tf.placeholder(tf.float32, [None, self.steps])
        ...
        #init_state = tf.fill(batch_size)
        init_state = tf.zeros([tf.reshape(batch_size,[]), self.my_rnn.state_size])
        with tf.variable_scope("RNN", reuse=None):
            for i in range(len(inputs)):           
                if i == 0:
                    (hidden, state) = self.my_rnn(inputs[i], init_state)
                else: 
                    m = x_mask[:, i]
        ...
        return x, x_mask, y, loss, train_op, accuracy, prediction
...
def train():
    ...
    os.chdir('/home/chuckgu/Desktop/project/Alphabeta/data/')
    train = pd.read_pickle('xproject_class_train.pkl')
    train_x = [ q for q in train['q'] ]
    train_y = [ a for a in train['a'] ]
    
    
    train_y = np.array(train_y)[:,None]
    train_y = to_categorical(train_y, config.y_size).astype('float32')
   ...
   # print 'train_x_img :', train_x_img.shape
    ...
    if config.valid_epoch is not None:
        valid=pd.read_pickle('xproject_class_test.pkl')    
        valid_x=[ q for q in valid['q'] ]
        valid_y=[ a for a in valid['a'] ]
        valid_y=np.array(valid_y)[:,None]
        valid_y = to_categorical(valid_y, config.y_size).astype('float32')
    ...
    with tf.Session() as sess:
        ...
        x, x_mask, y, loss, train_op, accuracy, prediction = model.build_model()
        ...
        for i in range(config.epoch):
            ...
            for j, indices in enumerate(batch_indices):
                ...
                #if j % 99 == 0 :
                print 'cost : ', cost, ', accuracy : ', acc, ', iter : ', j+1, ' in epoch : ',i+1, ' number : ', x_.shape
...
def test():                    
    ...
    test=pd.read_pickle('xproject_class_test.pkl')    
...
def test_sample(test_x,model_ckpt_path):                    
    ...
    with tf.Session() as sess:
        ...
        x, x_mask, _, _, _, _, prediction = model.build_model()
        saver = tf.train.Saver()        
        ckpt = tf.train.get_checkpoint_state(os.path.dirname(model_ckpt_path))
        saver.restore(sess, model_ckpt_path)


        test_x , test_x_mask = prepare_data(test_x, config.steps)
        '''
        x = np.zeros((1, config.steps)).astype('int32')
        x_mask = np.zeros((1, config.steps)).astype('float32')
        
        x[0, : len(test_x)] = test_x
        x_mask[0, :len(test_x)+1] = 1. # Adding 1, for image                
        '''
        x_ = test_x
        x_mask_ = test_x_mask
...
def build_test(model_ckpt_path):                    
    ...
    return x, x_mask,prediction
        
def test_data(test_x,x, x_mask,prediction,model_ckpt_path):                    
...
def get_config():
    class Config1(object):
        vocab_size = 900
        y_size = 83
        batch_size = 84 #703 #s28
        steps = 30

        #dim_ictx = 4096
        #dim_iemb = 1024 # image embedding
        dim_wemb = 128 # word embedding
        dim_hidden = 512
        epoch = 15
        
        lr = 0.001
        lr_decay = 0.9
        decay_epoch = 25. # epoch 보다 크면 decay 하지않음.
        
        dropout = 0.4
        
        rnn_type = 'gru'
        layers = None #it doesn’t work yet
        is_birnn = False #False
        valid_epoch = 1 # or None
        model_ckpt_path = '/home/chuckgu/Desktop/project/Alphabeta/save/model.ckpt'
    return Config1()

...
X_mask= np.ones((len(X_train),8)).astype("float32")
X_valid_mask= np.ones((len(X_valid),8)).astype("float32")
...
model.add(Flatten(dim=3))

model.add(GRU(50176,n_gru,return_seq=False))
...
model.compile(optimizer='SGD',loss='nll_multiclass',mask=True)
model.load_layers('temp/Video_pretrained.pkl')

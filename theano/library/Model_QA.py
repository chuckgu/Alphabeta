...
class NN_Model(object):   
    ...
    def __getstate__(self,best=False):
        """ Return state sequence."""
        if best:
            params = self.params  # parameters set in constructor
            weights = [p.get_value() for p in self.params_best]
        else:
            params = self.params  # parameters set in constructor
    ...
    def __setstate__(self, state):
        ...
        Parameters must be in the order defined by self.params:
            W, W_in, W_out, h0, bh, by
        """
        params, weights, error,val_error = state
    ...
    def save(self, fpath='temp/', fname=None, best=False):
        ...
        if best:
            print("Saving best model to %s ..." % fabspath)
            state = self.__getstate__(True)
        else:
    ...
    def get_output(self,train=False):
        ...
        #ctx=T.dot(self.img, self.W_hi) + self.b_hi
        
        self.layers[2].add_input(self.img)
        #self.layers[4].add_input(self.img)
        #self.layers[6].add_input(self.img)
    ...
    def compile(self,optimizer='Adam',loss='nll_multiclass',mask=True):      
       ...
       # self.params+=[self.W_hi, self.b_hi]        
        ...
        self.img = T.matrix(name = 'img', dtype = 'float32') 
        ...
        if mask:
            ...
            if type(self.X_train) == list:
                train_ins = self.X_train + self.X_train_mask+self.img+[self.y]
                test_ins = self.X_test + self.X_test_mask +self.img+ [self.y]
                predict_ins = self.X_test + self.X_test_mask +self.img
            else:
                train_ins = [self.X_train, self.X_train_mask, self.img, self.y]
                test_ins = [self.X_test, self.X_test_mask, self.img, self.y]
                predict_ins = [self.X_test, self.X_test_mask, self.img]
    ...
    def train(self,X_train,X_mask,Y_train,Img_train,X_val,X_val_mask,Y_val,Img_val,verbose=1,shuffle=True, show_accuracy=True):
        
        ### input data        
    
        train_set_x = np.asarray(X_train)
        train_set_y = np.asarray(Y_train)
        train_set_img=np.asarray(Img_train)
        
        if X_mask is None:
            ins = [train_set_x, train_set_y]
        else:
            mask_set_x = np.asarray(X_mask)            
            ins = [train_set_x, mask_set_x,train_set_img, train_set_y,]

        val_set_x = np.asarray(X_val)
        val_set_y = np.asarray(Y_val)   
        val_set_img=np.asarray(Img_val)  
        
        if X_val_mask is None:
            val_ins = [val_set_x, val_set_y]
        else:
            mask_val_set_x = np.asarray(X_val_mask)               
            val_ins = [val_set_x, mask_val_set_x,val_set_img, val_set_y]
        ...
        for epoch in range(self.n_epochs):
            ...
            if np.mod(epoch+1,self.val_Freq)==0:
                ...
                if epoch>0 and val_outs[1]<min(self.val_errors):           
                    self.params_best=self.params

                self.val_errors.append(val_outs[1])
            ...
            if np.mod(epoch+1,self.snapshot)==0:
                if epoch is not self.n_epochs: self.save(best=True)


        callbacks.on_train_end()   


    def evaluate(self, X_test,X_mask,Y_test,Img_test, batch_size=128, show_accuracy=False, verbose=1):
        
        test_set_x = np.asarray(X_test)
        test_set_y = np.asarray(Y_test)
        mask_set_x = np.asarray(X_mask)
        test_set_img=np.asarray(Img_test)
        
        if mask_set_x is None:
            ins = [test_set_x, test_set_y]
        else:
            ins = [test_set_x, mask_set_x,test_set_img, test_set_y]
    ...
    def _test_loop(self, f, ins, batch_size=128, verbose=0):
            ...
            if type(batch_outs) == list:
                if batch_index == 0:
                    for batch_out in enumerate(batch_outs):
                        outs.append(0.)
                for i, batch_out in enumerate(batch_outs):
                    outs[i] += batch_out * len(batch_ids) #sum of results
            else:
                if batch_index == 0:
                    outs.append(0.)
        ...
        for i, out in enumerate(outs):
            outs[i] /= nb_sample #divided by num of samples for averaging
        return outs

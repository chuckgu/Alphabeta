...
def img_to_array(img,norm=True,crop=True):
    import random
    x = np.asarray(img, dtype='float32')
    if crop:
        ix = random.randint(0,x.shape[1]-113)
        iy = random.randint(0,x.shape[0]-113)
        x = x[iy:iy+112,ix:ix+112]
...
def load_img(path,size=(160,120), grayscale=False):
    from PIL import Image
...
def load_list(path, batch_size, valid_portion=0.2):
...
def load_image(image_list):
    img=[]
    for frames in image_list:
        seq=[]
        for file_name in frames:
            seq.append(img_to_array(load_img(file_name)))
        img.append(seq)
